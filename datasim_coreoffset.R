...
for(i in 1:30){
  for(j in 1:100){
    g[j,i] <- w[j] * 0.5 + rnorm(n = 1, mean =0, sd = 0.5)
  }
}
...
for(i in 1:5){wgy$growth[wgy$tree == i] <- lead(wgy$growth[wgy$tree == i])}
ggplot(wgy, aes(x = w, y = growth, color = as.factor(tree))) +geom_point() + geom_smooth(method = "lm", se = FALSE)
...
cat("   
    ...
    for(i in 1:30){offset[i] ~ dbern(0.5) 
    }
...
mcmc_samples <- coda.samples(jags, variable.names=c("sigma","alpha", "offset"),  n.iter= 1000)
